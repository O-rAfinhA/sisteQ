...
export default function DocumentoCertidaoEditor() {
  ...
  const [formData, setFormData] = useState<Partial<DocumentoCertidao>>({
    tipoId: '',
    nomeDocumento: '',
    numeroProtocolo: '',
    orgaoEmissor: '',
  ...
  const calcularEAtualizarValidade = (dataEmissao: string, diasValidade?: number) => {
    ...
    if (diasValidade && diasValidade > 0) {
      ...
      if (!isEditMode || formData.status === 'Em Regularização') {
  ...
  const carregarDocumento = () => {
    const docs = getFromStorage<DocumentoCertidao[]>(STORAGE_KEY, []);
  ...
  const gerarNovoCodigo = (tipoId: string): string => {
    ...
    const prefixo = (tipo as any)?.prefixo || tipo?.prefixoCodigo || 'CERT-';
  ...
  const handleValidar = () => {
    if (!confirm('Confirma a validação desta certidão? Ela se tornará Regular.')) return;
  ...
  const handleSalvar = () => {
    if (!formData.tipoId || !formData.nomeDocumento || !formData.orgaoEmissor) {
    ...
    if (isEditMode) {
      ...
      if (index !== -1) {
        ...
        } as DocumentoCertidao;
      }
    } else {
      const novoDoc: DocumentoCertidao = {
        id: generateId(),
    ...
    <div className="p-8 max-w-4xl mx-auto space-y-8">
      <div>
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-gray-900 tracking-tight" style={{ fontSize: '1.75rem', fontWeight: 600, lineHeight: 1.3 }}>
            {isEditMode ? 'Editar Certidão' : 'Nova Certidão'}
          </h1>
        </div>
        <p className="text-gray-600">
          {isEditMode ? 'Atualize as informações da certidão' : 'Cadastre uma nova certidão ou regularidade fiscal'}
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <CardTitle>Informações da Certidão</CardTitle>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Nome da Certidão <span className="text-red-500">*</span>
              </label>
              <Input
            ...
            <div>
              ...
              {formData.status === 'Aguardando Validação' && (
                <p className="text-xs text-purple-600 mt-1 flex items-center gap-1">
                  ...
                  Requer validação para se tornar Regular
                </p>
              )}
            </div>
      ...
      <div className="flex gap-4 mt-6">
        <Button onClick={handleSalvar} variant={isEditMode ? "black" : "default"} className="flex-1 gap-2">
          ...
          {isEditMode ? 'Salvar Alterações' : 'Criar Certidão'}
        </Button>
        <Button
          variant="outline"
