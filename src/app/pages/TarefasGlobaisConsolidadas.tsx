...
import { Search, CheckCircle2, Clock, AlertCircle, ListTodo, Users, Building2, TrendingUp, Calendar, Eye, X } from 'lucide-react';
import { formatarData } from '../utils/formatters';
import { TarefaAcao, PlanoAcaoEstrategico, PlanoAcoes } from '../types/strategic';
import { useNavigate } from 'react-router';
import { MetricCard } from '../components/ui/metric-card';

interface TarefaConsolidada extends TarefaAcao {
  numeroPAE: string;
  numeroTarefa: string; // TE1, TE2... para PE ou TA1, TA2... para PA
  acaoPAE: string;
  planoId: string;
  tipoPlano: 'PE' | 'PA'; // Tipo do plano (PE = Plano Estratégico, PA = Plano de Ações)
}

type FiltroStatus = 'todas' | 'concluidas' | 'em-dia' | 'atrasadas' | 'vencem-7dias' | 'vencem-15dias' | 'vencem-30dias' | 'vencem-90dias';
type FiltroTipoPlano = 'todos' | 'PE' | 'PA';

export default function TarefasGlobaisConsolidadas() {
  const { dados, updatePlanoAcao, updatePlanoAcoes } = useStrategic();
  ...
  const [filtroTipoPlano, setFiltroTipoPlano] = useState<FiltroTipoPlano>('todos');
  const navigate = useNavigate();

  // Consolidar todas as tarefas de todos os PAEs (PE + PA)
  const tarefasConsolidadas = useMemo(() => {
    const tarefas: TarefaConsolidada[] = [];
    
    // Adicionar tarefas de Planos Estratégicos (PE)
    dados.planosAcao.forEach(plano => {
      plano.tarefas.forEach(tarefa => {
        tarefas.push({
          ...tarefa,
          numeroPAE: plano.numeroPE,
          numeroTarefa: `TE${plano.tarefas.indexOf(tarefa) + 1}`,
          acaoPAE: plano.acao,
          planoId: plano.id,
          tipoPlano: 'PE' // Plano Estratégico
        });
      });
    });
    
    // Adicionar tarefas de Planos de Ações (PA)
    dados.planosAcoes.forEach(plano => {
      plano.tarefas.forEach(tarefa => {
        tarefas.push({
          ...tarefa,
          numeroPAE: plano.numeroPE,
          numeroTarefa: `TA${plano.tarefas.indexOf(tarefa) + 1}`,
          acaoPAE: plano.descricaoNaoConformidade || 'Sem descrição',
          planoId: plano.id,
          tipoPlano: 'PA' // Plano de Ações
        });
      });
    });
  ...
  }, [dados.planosAcao, dados.planosAcoes]);
  ...
  const tarefasFiltradas = useMemo(() => {
    ...
    return tarefasConsolidadas.filter(tarefa => 
      ...
      tarefa.numeroPAE.toLowerCase().includes(buscaLower) ||
      tarefa.acaoPAE.toLowerCase().includes(buscaLower)
    );
  ...
  const toggleTarefa = (tarefa: TarefaConsolidada) => {
    if (tarefa.tipoPlano === 'PE') {
      // Atualizar tarefa de Plano Estratégico
      ...
      // Atualizar tarefa de Plano de Ações (PA)
      const plano = dados.planosAcoes.find(p => p.id === tarefa.planoId);
      if (!plano) return;
      ...
      updatePlanoAcoes(tarefa.planoId, { tarefas: tarefasAtualizadas });
    }
  };
  ...
  const verPlano = (tarefa: TarefaConsolidada) => {
    if (tarefa.tipoPlano === 'PE') {
      ...
      navigate(`/acoes-corretivas/plano-acao?planoId=${tarefa.planoId}`);
    }
  };
  ...
  const tarefasComFiltros = useMemo(() => {
    ...
    if (filtroTipoPlano !== 'todos') {
      resultado = resultado.filter(tarefa => tarefa.tipoPlano === filtroTipoPlano);
    }

    return resultado;
  }, [tarefasFiltradas, filtroStatus, filtroDepartamento, filtroResponsavel, filtroTipoPlano]);
  ...
  const limparFiltros = () => {
    ...
    setFiltroTipoPlano('todos');
    setBusca('');
  };

  // Verificar se há filtros ativos
  const temFiltrosAtivos = filtroStatus !== 'todas' || filtroDepartamento !== null || filtroResponsavel !== null || filtroTipoPlano !== 'todos' || busca.trim() !== '';

  return (
    <div className="p-8 max-w-[1400px] mx-auto space-y-8">
      <div>
        <h1 className="text-gray-900 tracking-tight" style={{ fontSize: '1.75rem', fontWeight: 600, lineHeight: 1.3 }}>Gestão de Tarefas Consolidadas</h1>
        <p className="text-gray-500 mt-1.5" style={{ fontSize: '0.9375rem', lineHeight: 1.6 }}>
          Visualização consolidada de todas as tarefas dos Planos Estratégicos (PE) e Planos de Ações (PA).
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <MetricCard
          ...
          icon={ListTodo}
          variant="default"
          onClick={() => { setFiltroStatus('todas'); setFiltroDepartamento(null); setFiltroResponsavel(null); setFiltroTipoPlano('todos'); }}
          active={filtroStatus === 'todas' && !filtroDepartamento && !filtroResponsavel && filtroTipoPlano === 'todos'}
        />
        <MetricCard
          label="Concluídas"
          ...
          icon={CheckCircle2}
          variant="success"
          onClick={() => { setFiltroStatus('concluidas'); setFiltroDepartamento(null); setFiltroResponsavel(null); setFiltroTipoPlano('todos'); }}
        ...
        <MetricCard
          ...
          icon={Clock}
          variant="info"
          onClick={() => { setFiltroStatus('em-dia'); setFiltroDepartamento(null); setFiltroResponsavel(null); setFiltroTipoPlano('todos'); }}
        ...
        <MetricCard
          ...
          icon={AlertCircle}
          variant="danger"
          onClick={() => { setFiltroStatus('atrasadas'); setFiltroDepartamento(null); setFiltroResponsavel(null); setFiltroTipoPlano('todos'); }}
      ...
      <div className="grid grid-cols-4 gap-4">
        <MetricCard
          ...
          icon={Clock}
          variant="warning"
          onClick={() => { setFiltroStatus('vencem-7dias'); setFiltroDepartamento(null); setFiltroResponsavel(null); setFiltroTipoPlano('todos'); }}
        ...
        <MetricCard
          ...
          icon={Clock}
          variant="warning"
          onClick={() => { setFiltroStatus('vencem-15dias'); setFiltroDepartamento(null); setFiltroResponsavel(null); setFiltroTipoPlano('todos'); }}
        ...
        <MetricCard
          ...
          icon={Clock}
          variant="warning"
          onClick={() => { setFiltroStatus('vencem-30dias'); setFiltroDepartamento(null); setFiltroResponsavel(null); setFiltroTipoPlano('todos'); }}
        ...
        <MetricCard
          ...
          icon={Clock}
          variant="warning"
          onClick={() => { setFiltroStatus('vencem-90dias'); setFiltroDepartamento(null); setFiltroResponsavel(null); setFiltroTipoPlano('todos'); }}
        ...
        <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {filtroTipoPlano !== 'todos' && (
                <Badge className="bg-white text-gray-700 border border-gray-300">
                  Tipo de Plano: {filtroTipoPlano === 'PE' ? 'Plano Estratégico' : 'Plano de Ações'}
                </Badge>
              )}
      ...
      {/* Campo de Busca e Filtro por Tipo de Plano */}
      <div className="mb-4 flex gap-3">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
          <Input
            type="text"
            placeholder="Buscar por descrição, responsável, departamento, PE/PA..."
            value={busca}
        ...
        <div className="flex gap-2">
          <Button
            variant={filtroTipoPlano === 'todos' ? 'default' : 'outline'}
            onClick={() => setFiltroTipoPlano('todos')}
            className="whitespace-nowrap"
          >
            Todos
          </Button>
          <Button
            variant={filtroTipoPlano === 'PE' ? 'default' : 'outline'}
            onClick={() => setFiltroTipoPlano('PE')}
            className="whitespace-nowrap bg-blue-600 hover:bg-blue-700 text-white"
            style={filtroTipoPlano === 'PE' ? {} : { backgroundColor: 'transparent', color: 'inherit' }}
          >
            PE
          </Button>
          <Button
            variant={filtroTipoPlano === 'PA' ? 'default' : 'outline'}
            onClick={() => setFiltroTipoPlano('PA')}
            className="whitespace-nowrap bg-purple-600 hover:bg-purple-700 text-white"
            style={filtroTipoPlano === 'PA' ? {} : { backgroundColor: 'transparent', color: 'inherit' }}
          >
            PA
          </Button>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50/60">
                    ...
                    <th className="px-4 py-3 text-left text-xs text-gray-500" style={{ fontWeight: 500 }}>PE/PA</th>
                        ...
                        <td className="px-4 py-3">
                          ...
                          >
                            <div className="flex flex-col items-start leading-tight">
                              ...
                              <span className="text-xs font-medium text-gray-500">{tarefa.numeroPAE}</span>
                            </div>
                          </button>
                        </td>
                        <td className="px-4 py-3">
